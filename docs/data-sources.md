...
- **"Does where each group lives explain the gap?" card, cross-checked against the same rural/urban composition data used on the Unemployment page** (`data/reference/population_by_sector_2017_18.json`, shared rather than re-transcribed): unlike unemployment, where population composition (Others being far more urban-concentrated, and urban unemployment running higher for every group) explains a real part of the ST/Others gap, the wealth gap does **not** reduce to composition — SC's asset value is roughly the same fraction of "Others"' in both rural (~34%) and urban (~32%) areas, so the gap holds up within each sector separately, not just in a blended national figure. Stated explicitly in the UI as a "mostly no" finding, to avoid implying the same explanation applies to every metric on this dashboard.
...
- **Documented release-suppression episode, disclosed in the UI**: the PLFS 2017-18 report (this dashboard's earliest data point) was withheld by the government past its own National Statistical Commission's approval; two non-government NSC members, including acting chairman P.C. Mohanan, resigned in January 2019 over it. The report leaked before being officially released, essentially unchanged, after the 2019 general election. Stated in a caveat banner on the page — this is about the source's release history, not evidence the underlying survey numbers were altered.
- **ST's unemployment rate is the lowest of any group in every year — flagged as a likely measurement artefact, not a finding of caste advantage**, backed by two supporting single-year tables in the UI: (1) population distribution by social group and sector (PLFS 2017-18, Statement 40) shows Others/General are far more urban-concentrated (40.3% of urban India vs. 28.2% nationally) than ST (3.6% urban vs. 9.3% nationally) — and unemployment runs structurally higher in urban areas for every group, so part of the gap is a rural/urban composition effect; (2) Labour Force Participation Rate and Worker Population Ratio by social group (PLFS 2023-24, Statements 17-18) show ST has the *highest* LFPR/WPR of any group, not the lowest — ruling out the theory that ST people are simply disengaged from the labour force. The likelier explanation: PLFS's "usual status" definition counts even marginal/subsistence work as "employed," and ST's concentration in rural agricultural work means most economically-active ST people get classified as employed regardless of how precarious that work is. Read alongside the Wealth page, where ST shows by far the lowest household asset value of any group — the same "low X isn't necessarily good news" pattern already flagged there for low debt incidence.
